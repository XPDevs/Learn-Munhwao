...
- Speaking tools: pronunciation scoring, speech recognition, practice lines and an AI conversation partner.
...
Any simple static server works. From the app folder run:
...
## AI partner and writing coach

The speaking partner and the writing coach run offline with rule based brains. To light them up with a real model, open Tools and then Settings and:

1. Tick the Use the AI partner box.
2. Enter the base address, for example https://api.openai.com/v1.
3. Enter your API key and pick a model. The default small model is a cheap choice.

The key is stored only in your browser and is sent only to the address you typed. The app calls the chat completions endpoint at that address.
